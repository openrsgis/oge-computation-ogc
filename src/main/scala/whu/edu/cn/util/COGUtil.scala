...
object BosCOGUtil{
   ...
   * @return 后端瓦片
   */
  def tileQuery(client: BosClient, level: Int, coverageMetadata: CoverageMetadata, windowsExtent: Extent, queryGeometry: Geometry, bandCounts: Int*): mutable.ArrayBuffer[RawTile] = {
    tmsLevel = level
    var bandCount = 1
    if (bandCounts.length > 1) throw new RuntimeException("bandCount 参数最多传一个")
    if (bandCounts.length == 1) bandCount = bandCounts(0)
