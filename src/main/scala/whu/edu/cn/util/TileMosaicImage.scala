...
object TileMosaicImage {
  ...
  def tileMosaic(TileRDDUnComputed: RDD[((Extent, SpaceTimeKey), List[RawTile])], method: String, tileLayerMetadata: TileLayerMetadata[SpaceTimeKey], dType: String): (RDD[(SpaceTimeBandKey, Tile)], TileLayerMetadata[SpaceTimeKey]) = {
    method match {
      case "min" => {
