...
public class COGHeaderParse {
    ...
    public static ArrayList<RawTile> tileQuery(int level, String in_path,
        ...
        try {
            ...
            InputStream inputStream = minioClient.getObject("oge", in_path, 0L,
                    (long) HEAD_SIZE);
            ...
            parse(headerByte, tileOffsets, cell, geoTrans, tileByteCounts, imageSize);
            System.out.println(cell);

            return getTiles(level, query_extent, crs, in_path, time, measurement, dType, resolution, productName, tileOffsets, cell, geoTrans, tileByteCounts);
    ...
    public static RawTile getTileBuf(RawTile tile) {
        try {
            ...
            tile.tileBuf = outStream.toByteArray();
            inputStream.close();
            outStream.close();
            return tile;
        } catch (MinioException | IOException | InvalidKeyException | NoSuchAlgorithmException e) {
