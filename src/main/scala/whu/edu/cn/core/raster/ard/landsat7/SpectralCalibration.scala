package whu.edu.cn.core.raster.ard.landsat7
...
object SpectralCalibration{
  ...
  def runWithGdal(implicit sc: SparkContext,
    ...
    rdd.foreach { x =>
      ...
      //There is no esun parameters for B6_VCID_1 and B6_VCID_2, so they will not be processed
      if(index != 6 && index != 7){
