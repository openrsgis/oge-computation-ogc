    ...
    satellite match {
      case "Landsat" =>{
        ...
        for(i <- 0 until 1){
          ...
          val parser = whu.edu.cn.core.raster.ard.landsat8.Parser(metaFile(0).getAbsolutePath, 11)
      ...
      case "Gaofen" =>{
        ...
        for(i <- 0 until 1){
          ...
          val parser = whu.edu.cn.core.raster.ard.gaofen1.Parser(metaFile(0).getAbsolutePath)
          parser.collectMetadata()
