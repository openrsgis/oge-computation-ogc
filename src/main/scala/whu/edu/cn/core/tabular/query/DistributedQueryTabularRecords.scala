...
import whu.edu.cn.core.cube.tabular.{TabularRDD, TabularRecord, TabularRecordRDD}
import whu.edu.cn.core.entity.BiDimensionKey.LocationTimeGenderKey
import whu.edu.cn.core.entity.{BiQueryParams, QueryParams, TabularGridLayerMetadata}
import whu.edu.cn.core.vector.grid.GridConf
import whu.edu.cn.util.{GISUtil, HbaseUtil, PostgresqlService, PostgresqlUtil}
import whu.edu.cn.core.tabular.grid.GridTransformer.getGeomGridInfo
...
object DistributedQueryTabularRecords {
  ...
  def getFactValues(table: String, factItem: String, items: Array[String], itemValues: Array[Array[Int]]): Array[Array[Int]] = {
    val connStr = PostgresqlUtil.url
    val conn = DriverManager.getConnection(connStr, PostgresqlUtil.user, PostgresqlUtil.password)
    val statement = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)
