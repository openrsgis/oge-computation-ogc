...
def PanSharp(coverage1: RDDImage, coverage2: RDDImage, method: String = "IHS", bandList: List[Short] = List(1, 2, 3), weightList: List[Double] = List(1, 1, 1)): RDDImage = {
  ...
  if (method == "IHS") {
    val RGB_IHS: DenseMatrix[Double] = DenseMatrix((1.0 / 3, 1.0 / 3, 1.0 / 3), (-Math.pow(2, 0.5) / 6, -Math.pow(2, 0.5) / 6, Math.pow(2, 1.5) / 6), (1 / Math.pow(2, 0.5), -1 / Math.pow(2, 0.5), 0.0))
    val IHS_RGB: DenseMatrix[Double] = DenseMatrix((1.0, -1 / Math.pow(2, 0.5), 1 / Math.pow(2, 0.5)), (1.0, -1 / Math.pow(2, 0.5), -1 / Math.pow(2, 0.5)), (1.0, Math.pow(2, 0.5), 0.0))
    newCoverageRdd = rdd.map { case (spatialKey, (multibandTile1, multibandTile2)) =>
      ...
      val RGB_matrix = DenseMatrix.zeros[Double](3, pixels) //先创建一个空的二维矩阵
      for (i <- 0 until 3) {
        val pixelArray: Array[Double] = multibandTile1.band(bandList(i) - 1).toArrayDouble()
        val pixelVector: DenseVector[Double] = DenseVector(pixelArray)
        RGB_matrix(i, ::) := pixelVector.t //DenseVector 默认是列向量，所以要转成行向量
      }
      //RGB->IHS
      val IHS_matrix: DenseMatrix[Double] = RGB_IHS * RGB_matrix
