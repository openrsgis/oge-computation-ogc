...
object CorrelationAnalysis {
  ...
  def corrMat(featureRDD: RDD[(String, (Geometry, mutable.Map[String, Any]))], properties: String, method: String = "pearson"): String = {
    ...
    val arrList = propertyArr.map(p => featureRDD.map(t => t._2._2("properties").asInstanceOf[com.alibaba.fastjson.JSONArray].getJSONObject(0).get(p).toString.toDouble).collect())
    if (method == "pearson") {
      cor = arrList.map(t => {
