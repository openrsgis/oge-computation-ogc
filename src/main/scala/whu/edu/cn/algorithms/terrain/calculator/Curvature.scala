...
object Curvature {
  ...
  def apply(
    ...
    val result = rddPadding.map(t => {
      val calculator = CurvatureCalculator(t._2.band(0), metaData, radius, zFactor)
  ...
  private case class CurvatureCalculator(tile: Tile, metaData: TileLayerMetadata[SpaceTimeKey],
                                         paddingSize: Int, zFactor: Double) {

    private val DEM: OgeTerrainTile = OgeTerrainTile.from(tile, metaData, paddingSize)
    private val Cur: OgeTerrainTile = OgeTerrainTile.like(tile, metaData, paddingSize)
