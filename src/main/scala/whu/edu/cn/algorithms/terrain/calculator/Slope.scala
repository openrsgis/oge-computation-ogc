...
object Slope {
  ...
  def apply(
    ...
    val result = rddPadding.map(t => {
      val calculator = SlopeCalculator(t._2.band(0), metaData, radius, zFactor)
      (t._1, MultibandTile(calculator()))
    })
    (result, metaData)
  }

  private case class SlopeCalculator(tile: Tile, metaData: TileLayerMetadata[SpaceTimeKey],
    ...
    private def doFdZevenbergen(x: Int, y: Int): Unit = {
      ...
      if (calSubMatrix(x, y, zm)) {
        ...
        Slope.setDouble(x, y, atan(sqrt(k2)).toDegrees)
      }
    }
